...
function GeometricDistribution(p0){
	...
	this.quantile = function(q){
		return Math.round(Math.log(1 - q)/Math.log(1 - this.prob()));
	}	
}
...
function WaldDistribution(m0, s0){
	var m, s, mx;
	if (0 < m0 && m0 < Infinity) m = m0; else m = 1;
	if (0 < s0 && s0 < Infinity) s = s0; else s = 1;
	mx = m + 3 * Math.sqrt(Math.pow(m,3)/s);
	Distribution.call(this, 0, mx, mx/100, CONT);

	this.density = function(x){
		if (x > 0) return Math.sqrt(s/(2*Math.PI*Math.pow(x,3)))*Math.exp(-s*Math.pow(x - m,2)/(2*Math.pow(m,2)*x));
		else return 0;
	};
	...
	this.mode = function(){
		return m*(Math.sqrt(1 + Math.pow(3*m/(2*s),2)) - 3*m/(2*s));
	};
	...
	this.simulate = function(){
		...
		var z = r * Math.cos(theta);
		var y = Math.pow(z,2);
		var x = m + (Math.pow(m,2)*y)/(2*s) - (m/(2*s))*Math.sqrt(4*m*s*y + Math.pow(m,2)*Math.pow(y,2));
		if (Math.random() <= m/(m+x)) w = x;
		else w = Math.pow(m,2)/x;
		this.setValue(w); 
		return w;
	};
...
WaldDistribution.prototype = new Distribution;
